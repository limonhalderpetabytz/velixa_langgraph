...
def register_user(email, full_name):
    ...
    If user does not exist, optionally create them.
    Return the valid sys_id for 'caller_id'.
    """
    # 1. Lookup user
    lookup_url = f"{INSTANCE}/api/now/table/sys_user?sysparm_query=email={email}"
    response = requests.get(lookup_url, auth=HTTPBasicAuth(USER, PASSWORD), headers={"Accept": "application/json"})
    ...
    if result:
        ...
        create_resp = requests.post(f"{INSTANCE}/api/now/table/sys_user",
                                    auth=HTTPBasicAuth(USER, PASSWORD),
                                    headers={"Content-Type": "application/json"},
                                    data=json.dumps(create_payload))
        create_result = create_resp.json().get('result', {})
        return {"sys_id": create_result.get('sys_id'), "email": email, "name": full_name}
...
def add_comments(user_input: str):
    ...
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    query_url = f"{INSTANCE}/api/now/table/incident?sysparm_query=number={ticket_id}"
...
def show_my_tickets(user_email: str):
    ...
    try:
        ...
        tickets_url = f"{INSTANCE}/api/now/table/incident"
...
def close_ticket(ticket_identifier: str, close_notes: str = "Issue resolved and ticket closed."):
    ...
    try:
        ...
        lookup_url = f"{INSTANCE}/api/now/table/incident"
...
user_tools = [submit_ticket, check_status, add_comments, submit_feedback, ask_question, reopen_ticket, show_my_tickets, close_ticket]
